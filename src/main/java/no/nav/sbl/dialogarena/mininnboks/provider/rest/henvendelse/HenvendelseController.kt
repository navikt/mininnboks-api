...
fun Route.HenvendelseController(henvendelseService: HenvendelseService, tilgangService: TilgangService) {
    ...
    route("/traader") {
        val logger = LoggerFactory.getLogger(HenvendelseController(henvendelseService, tilgangService)::class.java)
        ...
        get("/{id}") {
            ...
            val optionalTraad = hentTraad(henvendelseService, id, logger)
        ...
        post("/sporsmal") {
            ...
            val metrikk = MetricsFactory.createEvent("mininnboks.sendsporsmal")
        ...
        post("/svar") {
            ...
            val traadOptional = hentTraad(henvendelseService, svar.traadId, logger)
...
fun hentTraad(henvendelseService: HenvendelseService, id: String?, logger: Logger): Optional<Traad> {
    return try {
        ...
        Optional.empty()
    } catch (fault: SOAPFaultException) {
        logger.error("Fant ikke tråd med id: $id", fault)
        Optional.empty()
    }
}
