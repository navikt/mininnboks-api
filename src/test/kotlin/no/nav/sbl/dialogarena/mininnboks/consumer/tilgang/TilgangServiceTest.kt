...
object TilgangServiceTest : Spek({
    ...
    test("gir FEILET om hentEnhet feiler") {
        val (pdlService, tilgangService) = gittContext()
        coEvery { pdlService.hentGeografiskTilknytning(dummySubject) } throws IllegalStateException()
        runBlocking {
    ...
    test("gir INGEN_ENHET om bruker ikke har enhet") {
        val (pdlService, tilgangService) = gittContext()
        coEvery { pdlService.hentGeografiskTilknytning(dummySubject) } returns null
        runBlocking {
            val result = tilgangService.harTilgangTilKommunalInnsending(dummySubject)
    ...
    test("gir INGEN_ENHET om bruker har GT utland") {
        val (pdlService, tilgangService) = gittContext()
        runBlocking {
            coEvery { pdlService.hentGeografiskTilknytning(dummySubject) } returns "SWE"
            val result = tilgangService.harTilgangTilKommunalInnsending(dummySubject)
            assertThat(result.resultat, Matchers.`is`(TilgangDTO.Resultat.INGEN_ENHET))
    ...
    test("gir FEILET om pdl-api kall feiler") {
        val (pdlService, tilgangService) = gittContext()
        coEvery { pdlService.hentGeografiskTilknytning(dummySubject) } returns "0123"
...
private fun gittContext(): MockContext {
    ...
    val tilgangService = TilgangServiceImpl(pdlService)
    return MockContext(pdlService, tilgangService)
}
